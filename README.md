...
However, when the length of all lane segments are rather short, another, more simpler approach, can be used as well.
As shown in the example pictures below, a smoothening of the resulting reference line can also be archived by reducing the sampling point.
...
The result of this very rudimentary approximation approach are visualized below. It demonstrates, that despite the more complex cubic representation of the reference line the overall map representation is much simpler.
