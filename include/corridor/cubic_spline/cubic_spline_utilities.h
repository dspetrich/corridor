...
/**
 * @brief Project a point on a segment point
 *
 * @param data_segment
 * @param segment_info
 * @param point
 ...
 *        contain the projection of the point
 *
 * @param data: spline data matrix
 * @param point: which is projected on the spline
 * @return SegmentInfoVector<DataIdx, RealType>
 */

/**
 * @brief FindSegmentCandidates: returns all segment ids which are likely to
 *        contain the projection of the point. In the case that no pependicular
 *        projection onto the spline can be found, the point is either befor the
 *        first or after the last segment (or both, in case of a circle).
 *
 * @param data: spline data matrix
 * @param point: which is projected on the spline
 * @param segment_points: filled with the found segment candidates
 * @return true: if a perpendicular match is found
 * @return false: if no perpendicular match is found. In this case either the
 *                first or last segment or both segments are returned.
 */
